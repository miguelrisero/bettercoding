...
impl ServerHandler for McpServer {
    fn get_info(&self) -> ServerInfo {
        ...
        let preamble = match self.mode() {
            McpMode::Global => {
                "A BetterCoding MCP server for task, issue, repository, workspace, and session management."
            }
            McpMode::Orchestrator => {
                "An orchestrator-scoped BetterCoding MCP server with tools limited to the configured workspace and orchestrator session context."
            }
        };
        ...
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            // TODO(bc-legacy-cleanup): MCP implementation name is a protocol identity; keep stable.
