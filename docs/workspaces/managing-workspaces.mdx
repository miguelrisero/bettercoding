...
|----------|-----------------|
| **macOS** | `/var/folders/.../T/bettercoding/worktrees` (system temp) |
| **Linux** | `/var/tmp/bettercoding/worktrees` |
| **Windows** | `%TEMP%\bettercoding\worktrees` |

<Note>
The table shows fresh-install defaults. If an existing Vibe Kanban installation has worktree state in a legacy `vibe-kanban` directory, BetterCoding adopts that directory in place. Set `BC_WORKTREE_BASE` to hard-override the worktree location or `BC_DATA_DIR` to hard-override the application data location. At startup, check the **Resolved worktree base directory** and **Resolved data directory** log entries to see which paths were selected.
</Note>
...
<Tab title="macOS">
```bash
du -sh $TMPDIR/bettercoding/worktrees
```
</Tab>
<Tab title="Linux">
```bash
du -sh /var/tmp/bettercoding/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:TEMP\bettercoding\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
If you manually delete a worktree directory (e.g., using `rm -rf` in terminal), BetterCoding will **automatically recreate it** the next time you open that workspace. The worktree is recreated from the branch's last commit.
...
BetterCoding automatically cleans up "orphaned" worktrees on startup - these are worktree directories that no longer have a matching workspace in the database (e.g., if the app crashed during deletion).
